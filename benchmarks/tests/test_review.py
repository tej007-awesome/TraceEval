...
def test_build_review_queue_includes_both_fp_and_miss(tmp_path):
    ...
    content = build_review_queue(payload, tmp_path / "nonexistent_scenarios_dir")
    assert "Total review items: 2" in content
...
def test_build_review_queue_never_prefills_human_label(tmp_path):
    payload = _payload_with_one_fp_and_one_miss()
    content = build_review_queue(payload, tmp_path / "nonexistent_scenarios_dir")
    # The literal placeholder must appear once per entry, with no guessed value substituted.
...
def test_build_review_queue_empty_when_no_fp_or_miss(tmp_path):
    ...
    content = build_review_queue(payload, tmp_path / "nonexistent_scenarios_dir")
    assert "Total review items: 0" in content
